...
class SignatureAlgRsaSha256(object):
  ...
  def Sign(self, bytes_to_sign, logf=None):
    ...
    self._Log(logf, 'keypair size : %s' % self.keypair.size_in_bits())

    # Generate the PKCS1-v1_5 compatible message, which includes
    # magic ASN.1 bytes and padding:
    emsa_msg = self._MakeEmsaMessageSha256(bytes_to_sign, self.keypair.size_in_bits(), logf)
    ...
    h = SHA256.new(bytes_to_sign)
    signature_bytes = pkcs1_15.new(self.keypair).sign(h)
  ...
  def Verify(self, signed_bytes, signature_b64):
    ...
    h = SHA256.new(signed_bytes)
    print( "Verify:",signature_b64,putative_signature)
    print(pkcs1_15.new(self.keypair).verify(h, putative_signature))
    return pkcs1_15.new(self.keypair).verify(h, putative_signature)
